...
class ScanBarcodeFromCameraFragment : Fragment(), ConfirmBarcodeDialogFragment.Listener {
    ...
    private fun saveScannedBarcode(barcode: Barcode) {
        ...
        val dbUsers = FirebaseDatabase.getInstance().getReference("scan").child(userId)

        dbUsers.push().setValue(barcode.text)
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    // Toast.makeText(this, "Scan data saved to Firebase", Toast.LENGTH_SHORT).show()
                }
            }
    }
    ...
    private fun finishWithResult(result: Result) {
       ...
       val dbUsers = FirebaseDatabase.getInstance().getReference("scan")
        dbUsers.child(FirebaseAuth.getInstance().currentUser!!.uid)
            .child("dataScanned")
            .setValue(   result.text.toString())
